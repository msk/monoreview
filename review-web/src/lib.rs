...
/// Joins items of an iterator into a `String` with a separator, formatting each item
/// without allocating per-element intermediate strings.
///
/// This macro writes directly into a single `String` buffer via `write!`, avoiding `to_string()`/`format!()`
/// on each element.
///
/// Parameters
/// - `iter`: Any iterable producing items (e.g., `slice.iter()`, an iterator, or a mapped iterator).
/// - `sep`: The separator string inserted between formatted items (e.g., ",", ":", ", ").
/// - `|x| "{x}"`: A closure taking each element as `x` and a format string literal that can reference `x`.
///
/// Behavior
/// - Returns an empty `String` when the iterator is empty.
/// - Inserts `sep` between items, but not at the ends.
/// - Uses `enumerate().fold(...)` internally; complexity is linear in the number of items.
///
/// Examples
///
/// Joining numbers with commas
/// ```ignore
/// let s = map_join!([1, 2, 3], ", ", |x| "{x}");
/// assert_eq!(s, "1, 2, 3");
/// ```
///
/// Joining bytes as hex
/// ```ignore
/// let bytes = [0x01u8, 0x0Au8, 0xFFu8];
/// let s = map_join!(bytes.iter(), ":", |b| "{b:02x}");
/// assert_eq!(s, "01:0a:ff");
/// ```
///
/// Notes
/// - The format must be a string literal (macro restriction). For dynamic formatting,
///   pre-transform items with `.map(...)` before calling this macro.
/// - For more complex per-item logic, consider mapping the iterator first.
///
/// Panics
/// - If writing to the internal `String` fails; this should not happen under normal conditions.
#[macro_export]
macro_rules! map_join {
    ($iter:expr, $sep:expr, |$x:ident| $fmt:literal) => {{
        ...
        ($iter)
            ...
            .fold(String::new(), |mut acc, (i, $x)| {
                ...
                write!(&mut acc, $fmt).expect("Writing to String should never fail");
                acc
            })
    }};
}
...
mod tests {
    ...
    fn map_join_empty() {
        let v: Vec<u32> = vec![];
        assert_eq!(map_join!(v, ", ", |x| "{x}"), "");
    }

    #[test]
    fn map_join_numbers() {
        assert_eq!(map_join!(vec![1, 2, 3], ", ", |x| "{x}"), "1, 2, 3");
    }

    #[test]
    fn map_join_strs() {
        let v = vec!["a", "b", "c"];
        assert_eq!(map_join!(v, ", ", |x| "{x}"), "a, b, c");
    }

    #[test]
    fn map_join_custom_display() {
        let v = vec![S(7), S(9)];
        assert_eq!(map_join!(v, ", ", |x| "{x}"), "7, 9");
    }

    #[test]
    fn map_join_hex() {
        let v = vec![0x01u8, 0x0au8, 0xffu8];
        assert_eq!(map_join!(v, ":", |x| "{x:02x}"), "01:0a:ff");
    }
}
